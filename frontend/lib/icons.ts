...
export function getIconHtml(rowKind: string): { html: string; className: string } {
  if (rowKind === 'paid_dlc') return { html: iconSvg('dlc'), className: 'shc-row-icon-yellow' };
  if (rowKind === 'broken') return { html: iconSvg('broken'), className: 'shc-row-icon-yellow' };
  if (rowKind === 'conditional') return { html: iconSvg('conditional'), className: 'shc-row-icon-orange' };
  if (rowKind === 'unobtainable') return { html: iconSvg('unobtainable'), className: 'shc-row-icon-red' };

  if (rowKind === 'restricted') {
    return {
      html: '<i class="icon icon-fw icon-spinner shc-native-icon" aria-hidden="true"></i>',
      className: 'shc-row-icon-yellow',
    };
  }

  if (rowKind === 'players_perfected') return { html: iconSvg('star'), className: 'shc-row-icon-normal' };
  if (rowKind === 'median_completion') return { html: iconSvg('clock'), className: 'shc-row-icon-normal' };
  if (rowKind === 'perfected_by_starters') return { html: iconSvg('percent'), className: 'shc-row-icon-normal' };

  return { html: iconSvg('info'), className: 'shc-row-icon-muted' };
}

export function getValueClass(rowKind: string): string {
  if (rowKind === 'broken' || rowKind === 'paid_dlc' || rowKind === 'restricted') {
    return 'shc-value-yellow';
  }

  if (rowKind === 'conditional') {
    return 'shc-value-orange';
  }

  if (rowKind === 'unobtainable') {
    return 'shc-value-red';
  }
